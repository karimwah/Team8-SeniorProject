...
def prepare_carrier_segment(image, binary_message, start_loc): 
    ...
    image_vector = image_array.reshape(im_size) #turn image into 1D vector
    length_of_binary = len(binary_message)
    stop_loc = start_loc_int + length_of_binary
    carrier_segment = image_vector[start_loc_int:stop_loc] #extract the section of the 1D vectorized image
...
def insert_coded_segment(image_vector,coded_image_segment,start_loc):
    ...
    start_loc_int = int(start_loc)
    for x in range(0, l):
        coded_image_vector[start_loc_int +x]=coded_image_segment[x]
...
def get_coded_image_segment(c_image,start_loc_int,message_length):
    l = 8*(message_length)
    coded_image_segment = np.zeros(l, dtype='uint8')
    rows, cols, colors = c_image.shape
    im_size = rows*cols*colors
    coded_image_vector = c_image.reshape(im_size)
    stop_loc = start_loc_int+l
    for x in range(0,l):
        coded_image_segment[x] = coded_image_vector[start_loc_int+x]
    return(coded_image_segment)
